...
本物の ECDSA（P-256）では 256bit の巨大な数を扱うが、アルゴリズムは曲線のサイズに依存しない。ここでは `y² = x³ + x + 1 (mod 97)` という小さな曲線を使い、すべての値が 2 桁の整数に収まるようにした。

```javascript
const p = 97n;  // 曲線の法（有限体の素数）
const a = 1n;   // 曲線パラメータ a
const b = 1n;   // 曲線パラメータ b
// 曲線: y² = x³ + ax + b (mod p)
```
...
```
x = 0:  x³+x+1 mod 97 =  1  →  y² ≡  1 (mod 97)  →  y = 1, 96   →  (0,1), (0,96)
x = 1:  x³+x+1 mod 97 =  3  →  y² ≡  3 (mod 97)  →  y = 10, 87  →  (1,10), (1,87)
x = 2:  x³+x+1 mod 97 = 11  →  y² ≡ 11 (mod 97)  →  y = 37, 60  →  (2,37), (2,60)
x = 3:  x³+x+1 mod 97 = 31  →  y² ≡ 31 (mod 97)  →  y = 15, 82  →  (3,15), (3,82)
x = 4:  x³+x+1 mod 97 = 69  →  y² ≡ 69 (mod 97)  →  平方根なし（点が存在しない）
```

見つかるときは常に y と p−y のペア（10 + 87 = 97 = p）で、x 軸に対して対称な 2 点になる。この曲線には全部で 96 個の点がある（+ 無限遠点 ∞）。
...
この曲線では全 96 点の位数がすべて 97（素数）なので、どの点を G に選んでもよい。今回は G = (1, 10) を採用した。
...
const a = 1n;
const b = 1n;
const G = { x: 1n, y: 10n }; // 仕様として固定
```

G が曲線上にあることの検算: 10² mod 97 = 3、1³ + 1·1 + 1 mod 97 = 3。一致する。
...
秘密鍵 d を選び、公開鍵 Q = d · G を計算する。d は 1 から n-1 の範囲（この曲線では 1〜96）なら何でもよい。

```javascript
const n = findOrder(G, a, p); // n = 97
const d = 23n;                // 秘密鍵 (1 ≤ d ≤ n-1)
const Q = scalarMul(d, G, a, p); // 公開鍵 Q = d · G
console.log(`秘密鍵 d = ${d}`);
...
公開鍵 Q = (18, 15) から秘密鍵 d = 23 を逆算するのは、楕円曲線離散対数問題（ECDLP）であり、曲線が大きくなると計算量的に困難になる。
...
const h = toyHash(message, n);   // h = 33
const k = 37n;                   // 一時鍵（本番では暗号論的乱数）

const R = scalarMul(k, G, a, p); // R = (72, 29)
const r = mod(R.x, n);           // r = 72
const kInv = modInverse(k, n);   // k⁻¹ = 21
const s = mod(kInv * (h + r * d), n); // s = 64
...
h = 33
R = (72, 29)
k⁻¹ = 21  (検算: 37 × 21 mod 97 = 1)
s = 21 × (33 + 72 × 23) mod 97 = 64
署名 (r, s) = (72, 64)
```
...
```javascript
const sInv = modInverse(s, n);          // s⁻¹ = 47
const u1 = mod(h * sInv, n);           // u1 = 96
const u2 = mod(r * sInv, n);           // u2 = 86

const P1 = scalarMul(u1, G, a, p);     // P1 = (1, 87)
const P2 = scalarMul(u2, Q, a, p);     // P2 = (33, 88)
const Rp = pointAdd(P1, P2, a, p);     // R' = (72, 29)
...
const hBad = toyHash(tampered, n);        // h' = 63 (元の h = 33)

const u1b = mod(hBad * sInv, n);          // u1' = 51
const P1b = scalarMul(u1b, G, a, p);
const P2b = scalarMul(u2, Q, a, p);       // u2 は変わらず
const Rpb = pointAdd(P1b, P2b, a, p);     // R'' = (36, 6)
...
ハッシュ値が 33 → 63 に変わったことで、復元された点の x 座標が 36 になり、r = 72 と一致しなくなった。
...
const P1 = scalarMul(u1, G, a, p);
const P2 = scalarMul(u2, Q, a, p);
