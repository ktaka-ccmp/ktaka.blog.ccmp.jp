...
自社のコーポレートサイト（ccmp.jp）を、GCP 上の VM + Cloud Load Balancer + Cloud CDN 構成から GitHub Pages に移行しました。

もともと GitHub の有料プラン（Team）でリポジトリを管理していたため、GitHub Pages への移行で GCP 側の月額約 $23 のホスティングコストをまるごと削減できました。
...
現在の GCP 構成は 9 年前に構築したものです。当時としては、将来的な拡張性も考慮した妥当な設計でした。

一方で、この 9 年の間に状況は変わりました。

- VM の OS アップデート対応が地味に負担
- Terraform で再構築できるとはいえ、「本当に今も動くのか」という心理的な不安
- 静的サイト配信の選択肢が大きく増えた

実際の運用では動的機能を追加することはなく、静的配信のみが続いていました。「この規模のサイトに VM は本当に必要だろうか？」と考えるようになったのが、今回の移行のきっかけです。

なお、CMS として Joomla を使っているのは歴史的経緯によるものです。非エンジニアによる入稿を目的として 20 年近く継続利用してきました。9 年前にはすでに、CMS の本番公開をやめて `wget` による静的化へ移行していました。この「入稿と配信の分離」が、今回のホスティング移行を容易にしてくれました。
...
**GitHub Pages を採用しました**。決め手は apex ドメインでの IPv6 対応、ネイティブ Git 連携、ファイル名制約がない点です。なお、CDN については GitHub Pages も Fastly ベースとされる CDN を内蔵しており、Cloud CDN を別途構築する必要がなくなりました。
...
9 年前の設計は、当時の最適解でした。そしてその設計が、9 年後の移行コストを最小化してくれました。

今回の移行で改めて実感したのは、**入稿と配信を分離するアーキテクチャの強さ**です。Joomla CMS による入稿ワークフローは一切変更せず、ホスティング先だけを差し替えることで移行が完了しました。コンテンツ管理と配信基盤を疎結合にしておくことで、将来また別のホスティングサービスに移る場合でも同様にスムーズな移行ができます。
