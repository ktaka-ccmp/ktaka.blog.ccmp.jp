...
Screen の特徴として、「リージョン」と「ウィンドウ」が明確に分かれています。`Ctrl+a` → `S` で画面をリージョン分割し、各リージョンに既存のウィンドウを割り当てます。リージョンを閉じてもウィンドウ（シェル）は残ります。`Ctrl+a` → `Q` でリージョンを全解除して元の 1 画面に戻す、というのがよくある使い方でした。
