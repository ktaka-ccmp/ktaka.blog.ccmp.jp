...
なお、Screen では `Ctrl+a` → `S` は横分割（リージョン分割）です。この設定ではその機能を `-` に移しているため、`S` を「ペイン送り出し」に再割り当てしています。
...
| 操作 | Screen | tmux |
|---|---|---|
| デタッチ | `Ctrl+a` → `d` | `Ctrl+a` → `d` |
| セッション一覧 | `screen -ls` | `tmux ls` |
| アタッチ | `screen -r` | `tmux attach` (`tmux a`) |
| 名前付きセッション作成 | `screen -S name` | `tmux new -s name` |
| 名前指定でアタッチ | `screen -r name` | `tmux attach -t name` |
...
| `Ctrl+a` → `Q` (リージョン全解除) | `Ctrl+a` → `Q` (全ペイン分離、カスタム) | デフォルトの `z`（ズーム）は一時的な全画面化でペインが残る。カスタム `Q` で Screen に近い挙動を再現 |
...
移行のきっかけは Claude Code の表示崩れという消極的な理由でしたが、使ってみると tmux のセッション管理やペインの柔軟さは Screen より優れていると感じます。OSC 52 によるクリップボード連携のように、現代のターミナル環境は tmux を中心に進化しており、長年の慣れを差し引いても移行する価値はありました。Screen に慣れている方は、まずプレフィックスを `Ctrl+a` に変えるだけでも移行のハードルはかなり下がると思います。
