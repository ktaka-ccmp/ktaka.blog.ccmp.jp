# Issue: ブログ記事 — Web Crypto API で ECDSA 署名・検証を理解する
...
- [x] 記事ファイル作成（content/2026/WebCryptoEcdsa/index.md）
- [x] コードの動作確認（bun で実行、実行結果を記事に反映）
- [x] `zola build` でビルド確認
...
- Context: イシューの記事構成案をベースに執筆・レビューを実施
- Decision: 構成を調整（Web Crypto API 紹介 → 電子署名とは → コード の順、DPoP 言及は削除、generateKey アルゴリズム表を追加）
- Reason: 読者目線で「道具 → テーマ → 実践」の流れが自然。DPoP はこの記事単体では文脈がない

## Resolution

記事を公開した。ブランチ: `blog-webcrypto-01-ecdsa`（dev ベース）
