...
- The `data-ux_mode="popup"` defines the mode of Google's sign-in page, which we want it to be 'popup' instead of 'redirect' which is more intrusive.
- data-callback=onSignIn defines a JavaScript callback function upon a successful sign in on the Google side.
...
- create a session(, and stores it in a session database),
- set the session_id in the cookie,
- then return the response to the frontend.
